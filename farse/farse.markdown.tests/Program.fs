...
let benchmark file count =
  ...
  sw.Start()
  for n in 1 .. count do 
    Farse.Markdown.Html.Transform(text) |> ignore 
  sw.Stop()
...
let rec runTests dir = 
  for file in Directory.GetFiles(dir, "*.text") do
    transformAndCompare Farse.Markdown.Html.Transform file (file.Replace(".text", ".2.html")) (file.Replace(".text", ".html"))
  for file in Directory.GetFiles(dir, "*.text") do
    transformAndCompare Farse.Markdown.Xaml.Transform file (file.Replace(".text", ".2.xaml")) (file.Replace(".text", ".html"))
